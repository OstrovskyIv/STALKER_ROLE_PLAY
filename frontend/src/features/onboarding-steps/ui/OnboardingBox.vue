...
<template>
  <div v-if="activeStep" class="w-full max-w-5xl border-[3px] md:border-[5px] border-[#9241b8] shadow-[0_0_40px_rgba(146,65,184,0.2)] rounded-[2rem] md:rounded-[3rem] overflow-hidden flex flex-col relative transition-all duration-500 bg-transparent backdrop-blur-md">
    ...
    <div class="flex-1 relative overflow-hidden flex flex-col">
      ...
      <div class="relative z-10 flex-1 p-6 md:p-14 space-y-6 md:space-y-10 text-left">
        ...
        <p class="text-zinc-100 text-sm md:text-xl lg:text-2xl leading-tight max-w-5xl whitespace-pre-line font-sans font-black uppercase italic drop-shadow-md">{{ activeStep.desc }}</p>
        <a v-if="activeStep.link && activeStep.btnText" :href="activeStep.link" target="_blank" class="inline-block px-6 py-3 md:px-12 md:py-5 bg-[#9241b8] hover:bg-[#a85cd4] text-white font-capture text-sm md:text-2xl transition-all uppercase rounded-sm shadow-lg">{{ activeStep.btnText }}</a>
      </div>
    </div>
    <div class="h-14 md:h-20 bg-zinc-900/40 border-t-[2px] md:border-t-[4px] border-[#9241b8]/30 px-6 md:px-10 flex items-center justify-center gap-6 md:gap-16 relative z-20 font-capture">
      <button @click="prevStep" :disabled="currentStep === 1" class="group flex items-center disabled:opacity-0 transition-opacity">
        <div class="border-2 border-[#9241b8] px-4 py-1 md:px-8 md:py-2.5 flex items-center gap-2 text-white text-[10px] md:text-xl hover:bg-[#9241b8]/20 transition-all font-capture">
