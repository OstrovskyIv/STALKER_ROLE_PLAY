  ...
  <section class="w-full h-screen flex flex-col items-center relative snap-start shrink-0 overflow-hidden">
    ...
    <div class="w-full flex-1 flex flex-col items-center justify-center p-4 gap-10 md:gap-20 2xl:gap-32">
      ...
      <div class="w-full max-w-7xl flex flex-col gap-4 sm:gap-10 items-center 2xl:-translate-y-12 transition-transform duration-500">
        ...
        <div v-if="serverStore.isOnline && serverStore.serverData" class="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-3 sm:gap-4 2xl:gap-8 w-full max-w-lg 2xl:max-w-none px-4 sm:px-6">
          ...
          <div class="bg-black/40 border border-white/10 p-4 2xl:p-10 rounded-2xl 2xl:rounded-[2.5rem] backdrop-blur-xl flex flex-row 2xl:flex-col items-center justify-between 2xl:justify-center gap-4 shadow-xl overflow-hidden">
        ...
        <div v-else-if="!serverStore.isLoading" class="flex flex-col items-center gap-6 px-6 animate-fade-in">
          <div class="bg-red-500/10 border-2 border-red-500/40 p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] backdrop-blur-md flex flex-col items-center gap-6 shadow-[0_0_50px_rgba(239,68,68,0.2)]">
            ...
            <div class="flex flex-col gap-2 text-center">
              ...
              <p class="text-red-400 font-sans font-black italic text-sm md:text-xl uppercase">Включите VPN пожалуйста для получения данных</p>
            </div>
