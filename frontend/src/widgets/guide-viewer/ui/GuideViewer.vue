...
<template>
  <div class="w-full max-w-[1750px] h-[95vh] md:h-[90vh] flex flex-col lg:flex-row bg-black/40 border-2 border-white/10 rounded-[1.5rem] md:rounded-[3.5rem] overflow-hidden backdrop-blur-xl shadow-2xl">
    ...
    <div ref="contentRef" class="flex-1 overflow-y-auto no-scrollbar p-4 xl:p-12 bg-black/10 relative scroll-smooth">
      <div v-if="filteredGuides.length"
           :class="['grid gap-6 xl:gap-10', activeCategory === 'blowout' ? 'grid-cols-1' : 'grid-cols-1 2xl:grid-cols-2']">
        ...
        <div v-for="guide in filteredGuides" :key="guide.id"
          ...
          <div class="relative h-full w-full bg-[#0a0a0a] rounded-[1.9rem] xl:rounded-[2.4rem] p-5 xl:p-10 flex flex-col gap-6 xl:gap-10 z-10">

            <div :class="['flex flex-col gap-8 items-center', activeCategory === 'blowout' ? 'lg:flex-row' : 'sm:flex-row']">
              <div class="relative shrink-0">
                <div :class="[
                  'rounded-2xl xl:rounded-3xl overflow-hidden border-2 transition-all duration-500 group-hover:scale-105',
                  isExtreme(guide.lethality) ? 'border-[#9241b8] shadow-[0_0_30px_rgba(146,65,184,0.4)]' : 'border-white/10',
                  activeCategory === 'blowout' ? 'w-64 h-64 md:w-80 md:h-80' : 'w-32 h-32 xl:w-48 xl:h-48'
                ]">
                  <img :src="getImageUrl(guide.imageName)" :alt="guide.title" class="w-full h-full object-cover bg-zinc-900/30" loading="lazy" />
                </div>
              ...
              <div class="flex-1 flex flex-col gap-4 text-center sm:text-left">
                <div class="border-l-4 border-[#9241b8] pl-5 flex flex-col gap-2">
                  <h3 :class="['text-white font-capture uppercase italic leading-none', activeCategory === 'blowout' ? 'text-3xl md:text-6xl' : 'text-xl xl:text-4xl']"
                      v-html="highlightText(guide.title, searchQuery)"></h3>
                ...
                <p :class="['text-zinc-400 font-sans italic leading-relaxed opacity-80', activeCategory === 'blowout' ? 'text-sm md:text-xl' : 'text-xs xl:text-base']"
                   v-html="highlightText(guide.description, searchQuery)"></p>
              </div>
            </div>

            <div :class="['grid gap-4', activeCategory === 'blowout' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2']">
              <div v-if="guide.features" class="bg-white/[0.03] border border-white/5 p-4 xl:p-6 rounded-2xl flex flex-col gap-2">
                <span class="text-[#9241b8] font-capture text-[10px] xl:text-xs uppercase tracking-widest opacity-60">Особенности / Защита</span>
                <span :class="['text-zinc-200 font-sans italic leading-tight', activeCategory === 'blowout' ? 'text-xs md:text-lg' : 'text-[10px] xl:text-xs']">{{ guide.features }}</span>
              </div>
              <div v-if="guide.trophies" class="bg-emerald-500/[0.03] border border-emerald-500/10 p-4 xl:p-6 rounded-2xl flex flex-col gap-2">
                <span class="text-emerald-500 font-capture text-[10px] xl:text-xs uppercase tracking-widest opacity-60">Трофеи / Артефакты</span>
                <span :class="['text-zinc-200 font-sans italic leading-tight', activeCategory === 'blowout' ? 'text-xs md:text-lg' : 'text-[10px] xl:text-xs']">{{ guide.trophies }}</span>
              </div>
              <div v-if="guide.danger" :class="['bg-red-500/[0.03] border border-red-500/10 p-4 xl:p-6 rounded-2xl flex flex-col gap-2', activeCategory === 'blowout' ? 'lg:col-span-2' : 'sm:col-span-2']">
                <span class="text-red-500 font-capture text-[10px] xl:text-xs uppercase tracking-widest opacity-60">Поведение / Последствия</span>
                <span :class="['text-zinc-200 font-sans italic leading-tight', activeCategory === 'blowout' ? 'text-xs md:text-lg' : 'text-[10px] xl:text-xs']">{{ guide.danger }}</span>
              </div>
            </div>
